...
class Flash2ViaSS_normal(Flash2):
    ...
    def solve(self, K0):
        ...
        while np.any(diff > self.tol) and cont < self.contmax:
            ...
            for i in range(N_points):
                ...
                resp = optimize.root(self.flash_equation, x0=0.5, args=(zi, Ki))
        ...
        for i in range(len(temperature)):
            if v[i] >= 1.0:
                ...
                flowrate_V_flatten[:,i] = np.diag(stream.components_properties.MM*1e-3) @ (y_flatten[:,i] * total_molar_flowrate_flatten[i]* (1-1e-10))
                flowrate_L_flatten[:,i] = np.diag(stream.components_properties.MM*1e-3) @ (y_flatten[:,i] * total_molar_flowrate_flatten[i] * 1e-10)
            elif v[i] <= 0.0:
                v[i] = 0.0
                x_flatten[:,i] = composition[:,i]
                y_flatten[:,i] = composition[:,i]
                flowrate_L_flatten[:,i] = np.diag(stream.components_properties.MM*1e-3) @ (x_flatten[:,i] * total_molar_flowrate_flatten[i]*(1-1e-10))
                flowrate_V_flatten[:,i] = np.diag(stream.components_properties.MM*1e-3) @ (x_flatten[:,i] * total_molar_flowrate_flatten[i] * 1e-10)
            elif 0.0 < v[i] < 1.0:
